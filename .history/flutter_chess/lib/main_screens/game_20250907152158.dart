...
import 'package:flutter_chess/widgets/game_board.dart';
      ...
      if (result) {
        ...
        if (gameProvider.state.state == PlayState.theirTurn && !gameProvider.aiThinking) {
          ...
          if (bestMove != null && context.mounted) {
            final aiMove = Move.fromUci(bestMove, gameProvider.game.board.size);
            gameProvider.makeSquaresMove(aiMove);
            await gameProvider.setSquaresState().whenComplete(() {
              if (gameProvider.player == Squares.white) {
                gameProvider.pauseBlacksTimer();
                gameProvider.startWhitesTime(context: context, onNewGame: () {});
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          if (gameProvider.state.state == PlayState.gameOver)
            IconButton(
          ...
          Expanded(
            child: Center(
              child: GameBoard(
                ...
                onMove: _onMove,
                flipBoard: gameProvider.flipBoard,
              ),
            ),
          ),
          Padding(
