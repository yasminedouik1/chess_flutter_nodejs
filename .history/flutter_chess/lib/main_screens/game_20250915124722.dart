...
class GameScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> handleMultiplayerMove(BuildContext context, Move move) async {
      ...
      final isValidMove = legalMoves.any(
      ...
      if (result) {
        ...
        print(
          'Sending move: $move, FEN: $newFen, isWhite: ${gameProvider.isHumanWhite}',
        );
    ...
    Future<void> onMove(Move move) async {
      print(
        'Player attempted move: $move, PlayState: ${gameProvider.state.state}',
      );
    ...
    useEffect(() {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        print(
          'Initializing GameScreen, vsComputer: ${gameProvider.vsComputer}, isPlaying: ${gameProvider.isPlaying}',
        );
        ...
        if (gameProvider.vsComputer) {
          ...
          gameProvider.setSquaresState().then((_) {
            print(
              'Multiplayer board state initialized: PlayState=${gameProvider.state.state}, Player=${gameProvider.player}',
            );
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (!gameProvider.vsComputer) ...[
            ElevatedButton(
              onPressed: gameProvider.drawOffered
                  ? null
                  : () => gameProvider.offerDraw(context),
