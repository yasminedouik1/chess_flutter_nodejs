...
class _GameStartUpScreenState extends State<GameStartUpScreen> {
  ...
  Future<void> _createGame(GameProvider gameProvider, AuthProvider authProvider) async {
    ...
    if (user == null || authProvider.token == null) {
      showSnackBar(context: context, content: 'Please log in to create a game', duration: const Duration(seconds: 2));
      return;
    }
    ...
    try {
      ...
      if (widget.isCustomTime) {
        if (whiteTimeInMinutes < 1 || blackTimeInMinutes < 1) {
          showSnackBar(context: context, content: 'Time must be at least 1 minute', duration: const Duration(seconds: 2));
          return;
        }
        if (incrementalValue < 0) {
          showSnackBar(context: context, content: 'Increment cannot be negative', duration: const Duration(seconds: 2));
          return;
        }
        ...
        try {
          ...
          showSnackBar(context: context, content: 'Invalid game time format', duration: const Duration(seconds: 2));
          return;
        }
      }
      ...
      showSnackBar(context: context, content: 'Error creating game: $e', duration: const Duration(seconds: 2));
    } finally {
