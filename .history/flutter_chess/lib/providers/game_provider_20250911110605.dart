...
class GameProvider extends ChangeNotifier {
  ...
  Future<void> createGame({
      ...
      String? joinCode,  // Only for private games
    }) async {
      try {
        _isLoading = true;
        notifyListeners();
        ...
        final data = await ApiService.createGame(
          ...
          blackTime: blackTime,
          isPrivate: isPrivate,
          joinCode: isPrivate ? joinCode : null,  // Fix: Only pass joinCode for private
        );

        _gameId = data['gameId'];
        _joinCode = isPrivate ? (data['joinCode'] ?? '') : '';  // Fix: Empty for public games
        _isHumanWhite = playerColor == PlayerColor.white;
        _whitesTime = Duration(minutes: whiteTime);
        _blacksTime = Duration(minutes: blackTime);
        _isPlaying = false;

        ApiService.joinGameRoom(_gameId);  // Join socket room
  ...
  Future<void> cancelGame(BuildContext context) async {
      try {
        final response = await http.post(
          Uri.parse('$baseUrl/games/cancel/$_gameId'),
        ...
        if (response.statusCode == 200) {
          ...
          if (context.mounted) {
            showSnackBar(context: context, content: 'Game cancelled and deleted');
            Navigator.pushNamedAndRemoveUntil(context, Constants.homeScreen, (route) => false);
          }
        } else {
          throw Exception('Failed to cancel: ${_safeDecode(response.body)['message']}');
        }
      } catch (e) {
        if (context.mounted) {
  ...
  void initSocketListeners(BuildContext context) {
      ApiService.onPlayerJoined((data) {
        final gameId = data['gameId'];
        if (gameId == _gameId) {
          _opponentId = data['opponentId'];
          _opponentName = data['opponentName'];
          _opponentImage = data['opponentImage'] ?? '';
          _opponentRating = data['opponentRating'] ?? 1200;
          _isPlaying = true;  // Fix: Set isPlaying for player 1
          notifyListeners();

          if (context.mounted) {
            Navigator.pushReplacementNamed(context, Constants.gameScreen);  // Navigate player 1
          }
        }
      });
