...
class GameProvider extends ChangeNotifier {
  ...
  void setOpponentData({
    ...
    _opponentName = opponentName.isNotEmpty ? opponentName : 'Opponent';
...
Future<void> createGame({
    ...
    try {
      ...
      if (response['gameId'] == null) {
        throw Exception('Game ID not returned from server');
      }
      ...
      showSnackBar(context: context, content: 'Failed to create game: $e', duration: const Duration(seconds: 2));
      throw e;
    }
  }
...
Future<void> cancelGame(BuildContext context) async {
    if (_gameId.isNotEmpty && context.read<AuthProvider>().token != null) {
      try {
        ...
        showSnackBar(context: context, content: 'Error cancelling game: $e', duration: const Duration(seconds: 2));
      }
    }
  }
  ...
  void initSocketListeners(BuildContext context) {
    ...
    ApiService.socket?.on('draw_declined', (_) {
      _drawOffered = false;
      notifyListeners();
      showSnackBar(context: context, content: 'Draw offer declined', duration: const Duration(seconds: 2));
    });
    ...
    ApiService.socket?.on('invalid_move', (data) {
      showSnackBar(context: context, content: data['message'], duration: const Duration(seconds: 2));
    });
  ...
  Future<void> playMove({
    ...
    if (!_vsComputer && _gameId.isNotEmpty && token != null) {
      ...
      showSnackBar(
        context: context,
        content: 'No token available. Please log in.', duration: const Duration(seconds: 2)
      );
    }
  }
