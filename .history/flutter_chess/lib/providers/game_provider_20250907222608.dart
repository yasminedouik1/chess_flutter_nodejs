...
void initSocketListeners(BuildContext context) {
    ApiService.onOpponentJoined(context, (data) {
      ...
      _blacksTime = Duration(seconds: data['blackTime']);
     _isPlaying = true;
      _isHumanWhite = data['creatorId'] == ApiService.getUserId() ? true : false;
      notifyListeners();
    });
    ...
    ApiService.onMoveReceived(context, (data) {
      ...
      if (state.board.fen != fen) {
        _state = state.copyWith(board: chess);
        if (isWhiteMove && player == Squares.black || !isWhiteMove && player == Squares.white) {
          _state = state.copyWith(state: PlayState.ourTurn);
        } else {
          _state = state.copyWith(state: PlayState.theirTurn);
        }
        notifyListeners();
      }
