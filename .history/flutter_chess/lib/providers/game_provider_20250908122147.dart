...
set gameId(String? value) {
  ...
  String squareToAlgebraic(int index) {
   ...
   Move _bishopMoveToSquaresMove(bishop.Move move) {
    String algebraic = move.algebraic(_game.board);
    final from = _algebraicToIndex(algebraic.substring(0, 2));
    final to = _algebraicToIndex(algebraic.substring(2, 4));
    final promotion = algebraic.length > 4 ? algebraic[4].toUpperCase() : null;
    return Move(
      from: from,
      to: to,
      promo: promotion,
