...
// Convert algebraic notation (e.g., 'e2') to board index (0-63)
  ...
  Future<void> createGame({
    required UserModel user,  // Use your UserModel type here, e.g., required UserModel user
    required int whiteTime,
    required int blackTime,
    required int increment,
    ...
    try {
      // Fix: Use concrete type instead of dynamic
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      final token = authProvider.token;

      if (token == null) {
        onFail('No authentication token available. Please log in.');
        return;
      }

      // Call your ApiService (adjust params if needed based on your exact ApiService impl)
...
final gameId = responseData['gameId'] as String;
    final joinCode = isPrivate ? (responseData['joinCode'] as String?) ?? '' : '';
      // Update provider state on success
      _gameId = gameId;
      _userId = user.uid;  // Adjust based on your UserModel
      _isPrivate = isPrivate;
      _isHumanWhite = true;  // Creator is white, as per waiting_screen.dart
      _savedWhitesTime = Duration(seconds: whiteTime);
      _savedBlacksTime = Duration(seconds: blackTime);
      _incrementalValue = increment;
      _whitesTime = _savedWhitesTime;
      ...
      // If private, set join code (assume ApiService or response provides it; implement if needed)
    if (isPrivate && joinCode.isNotEmpty) {
      _joinCode = joinCode;
    }

    // Initialize socket for PvP
...
void initSocketListeners(BuildContext context) {
    ApiService.onOpponentJoined(context, (data) {
      ...
      if (_game.makeMoveString(move)) {
        ...
        setState(SquaresState(
          board: _game.squaresState(_player).board,
          player: _isHumanWhite ? Squares.white : Squares.black,
          state: isWhiteMove && _player == Squares.black || !isWhiteMove && _player == Squares.white
              ? PlayState.ourTurn
              : PlayState.theirTurn,
          size: BoardSize(8, 8),
          moves: legalMoves,
      ...
      if (checkmate(fen, context)) {
        gameOverDialog(
          context: context,
          timeOut: false,
          userWon: (isWhiteMove && _player == Squares.black) || (!isWhiteMove && _player == Squares.white),
          onNewGame: () {},
          reason: 'checkmate',
        );
      }
