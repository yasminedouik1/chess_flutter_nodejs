...
class GameProvider extends ChangeNotifier {
  ...
  Future<void> setSquaresState() async {
    _state = SquaresState(
      board: _game.board,
      player: _player,
      moves: _game.generateLegalMoves(),
      state: _game.state,
    );
    notifyListeners();
  }
      ...
      if (_whitesTime.inSeconds > 0) {
        ...
        if (!_vsComputer) {
          ApiService.socket?.emit('game_over', {
            'gameId': _gameId,
            'reason': 'timeout',
            'winnerId': _isHumanWhite ? _opponentId : _userId,
          });
        }
      }
    });
      ...
      if (_blacksTime.inSeconds > 0) {
        ...
        if (!_vsComputer) {
          ApiService.socket?.emit('game_over', {
            'gameId': _gameId,
            'reason': 'timeout',
            'winnerId': _isHumanWhite ? _userId : _opponentId,
          });
        }
      }
    });
  ...
  void gameOverListener({required BuildContext context, required Function onNewGame}) {
    if (_game.state == bishop.State.checkmate) {
      final userWon = (_isHumanWhite && _player == Squares.black) || (!_isHumanWhite && _player == Squares.white);
      gameOverDialog(
        context: context,
        timeOut: false,
        userWon: userWon,
      ...
      if (!_vsComputer) {
        ApiService.socket?.emit('game_over', {
          'gameId': _gameId,
          'reason': 'checkmate',
          'winnerId': userWon ? _userId : _opponentId,
        });
      }
    } else if (_game.state == bishop.State.stalemate || _game.state == bishop.State.draw) {
      gameOverDialog(
        context: context,
        timeOut: false,
        userWon: false,
  ...
  void listenToSocketEvents(BuildContext context) {
    ...
    ApiService.socket?.on('game_over', (data) {
      ...
      final userWon = winnerId == _userId;
      gameOverDialog(
        context: context,
...
//     final moveStr = "${move.from}-${move.to}";
//     ApiService.socket!.emit('move', {
//       'gameId': _gameId,
//       'move': moveStr,
//       'isWhite': _player == Squares.white,
//       'fen': _game.fen,
//     });
//   }
//   notifyListeners();
...
//         _userId = user.uid;
//         _isPlaying = true;
//         ApiService.socket?.emit('join_game', {
//           'gameId': _gameId,
//           'userId': _userId,
//         });
//         onSuccess();
