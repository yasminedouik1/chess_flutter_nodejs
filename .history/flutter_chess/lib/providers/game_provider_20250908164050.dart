...
Future<void> createGame({
  ...
  try {
    ...
    if (token == null) {
      onFail('No authentication token. Please log in.');
      return;
    }

    print('Creating game for user: ${user.uid}, isPrivate: $isPrivate'); // Debug log

    // Call ApiService
    final responseData = await ApiService.createGame(
      token: token,
      whiteTime: whiteTime,
      blackTime: blackTime,
      increment: increment,
    ...
    _gameId = gameId;
    _userId = user.uid; // Matches UserModel
    ...
    print('Game created: ID=$gameId, JoinCode=$joinCode'); // Debug log
