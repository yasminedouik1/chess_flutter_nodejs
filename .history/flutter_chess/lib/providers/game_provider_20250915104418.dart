...
void gameOverListener({
    ...
    if (_game.gameOver) {
      ...
      if (_game.checkmate) {
        reason = 'checkmate';
        final winnerColor = _game.winner;
        winnerId = _vsComputer
            ? null // No user ID for computer games
            : (winnerColor == Squares.white)
    ...
    if (timeOut) {
      ...
      if (_vsComputer) {
        if (userWon) {
          // Human wins
          if (_isHumanWhite) {
            _whitesScore = (_whitesScore + 1.0).clamp(0.0, double.infinity);
          } else {
            _blacksScore = (_blacksScore + 1.0).clamp(0.0, double.infinity);
          }
        } else {
          // Computer wins
          if (_isHumanWhite) {
      ...
      if (_game.drawn || _game.stalemate) {
        _whitesScore += 0.5;
        _blacksScore += 0.5;
      } else if (_game.winner == Squares.white) {
        _whitesScore = (_whitesScore + 1.0).clamp(0.0, double.infinity);
      } else if (_game.winner == Squares.black) {
        _blacksScore = (_blacksScore + 1.0).clamp(0.0, double.infinity);
      }
    }
