...
class GameProvider extends ChangeNotifier {
  ...
  void resetGame({required bool newGame, required BuildContext context}) {
    ...
    print('resetGame: Initial FEN=${_game.fen}, BoardSize=${_state.board..board}, PlayState=${_state.state}');
  ...
  Future<void> setSquaresState() async {
    ...
    _state = SquaresState(
      board: _game.squaresState(_player).board,
      player: _player,
      state: _game.turn == bishop.Bishop.white && _isHumanWhite || _game.turn == bishop.Bishop.black && !_isHumanWhite
          ? PlayState.ourTurn
          : PlayState.theirTurn,
      size: BoardSize(8, 8),
      moves: legalMoves,
    );
    print('setSquaresState: FEN=${_game.fen}, PlayState=${_state.state}, LegalMoves=${legalMoves.length}, BoardSize=${_state.board.board.length}');
    notifyListeners();
  }
  ...
  Future<void> setGameTime({
    ...
    try {
      final whiteMinutes = int.tryParse(newSavedWhitesTime.replaceAll(RegExp(r'[^0-9]'), '')) ?? 10;
      final blackMinutes = int.tryParse(newSavedBlacksTime.replaceAll(RegExp(r'[^0-9]'), '')) ?? 10;
      ...
      print('setGameTime: whiteMinutes=$whiteMinutes, blackMinutes=$blackMinutes');
      notifyListeners();
    } catch (e) {
      print('setGameTime: Error parsing time - whiteTime=$newSavedWhitesTime, blackTime=$newSavedBlacksTime, error=$e');
      // Fallback to default values
      _savedWhitesTime = const Duration(minutes: 10);
      _savedBlacksTime = const Duration(minutes: 10);
      setWhitesTime(_savedWhitesTime);
      setBlacksTime(_savedBlacksTime);
      notifyListeners();
    }
  ...
  Future<void> createGame({
    ...
    try {
      ...
      _gameId = response['gameId']?.toString() ?? '';
      _joinCode = response['joinCode']?.toString() ?? '';
      ...
      print('createGame: Game created with ID=$_gameId, joinCode=$_joinCode, isHumanWhite=$_isHumanWhite, FEN=${_game.fen}');
  ...
  Future<void> createComputerGame({
    ...
    try {
      ...
      await initStockfish();
      resetGame(newGame: true, context: context);
      print('createComputerGame: Initialized, FEN=${_game.fen}, isHumanWhite=$_isHumanWhite');
      Navigator.pushNamed(context, Constants.gameScreen);
    } catch (e) {
      print('createComputerGame: Error=$e');
  ...
  Future<void> joinGame(BuildContext context, String gameId, String userId) async {
    ...
    try {
      ...
      _gameId = gameData['gameId']?.toString() ?? '';
      _opponentId = gameData['creatorId']?.toString() ?? '';
      _opponentName = gameData['creatorName']?.toString() ?? 'Opponent';
      _opponentImage = gameData['creatorImage']?.toString() ?? '';
      _opponentRating = int.tryParse(gameData['creatorRating']?.toString() ?? '1200') ?? 1200;
      _whitesTime = Duration(seconds: int.tryParse(gameData['whiteTime']?.toString() ?? '600') ?? 600);
      _blacksTime = Duration(seconds: int.tryParse(gameData['blackTime']?.toString() ?? '600') ?? 600);
      _savedWhitesTime = _whitesTime;
      _savedBlacksTime = _blacksTime;
      _isHumanWhite = false;
      ...
      _game = bishop.Game(variant: bishop.Variant.standard());
      await setSquaresState();
      print('joinGame: Initialized board, FEN=${_game.fen}, PlayState=${_state.state}, whiteTime=${_whitesTime.inSeconds}s, blackTime=${_blacksTime.inSeconds}s');
      ApiService.initializeSocket(token);
      ApiService.joinGameRoom(_gameId);
  ...
  Future<void> searchGame({
    ...
    try {
      if (isJoin && joinGameId != null) {
        ...
        _gameId = game['game']['gameId']?.toString() ?? '';
        _opponentId = game['game']['creatorId']?.toString() ?? '';
        _opponentName = game['game']['creatorName']?.toString() ?? 'Opponent';
        _opponentImage = game['game']['creatorImage']?.toString() ?? '';
        _opponentRating = int.tryParse(game['game']['creatorRating']?.toString() ?? '1200') ?? 1200;
        _userId = user.uid;
        _isHumanWhite = false;
        _player = Squares.black;
        _isPlaying = true;
        _whitesTime = Duration(seconds: int.tryParse(game['game']['whiteTime']?.toString() ?? '600') ?? 600);
        _blacksTime = Duration(seconds: int.tryParse(game['game']['blackTime']?.toString() ?? '600') ?? 600);
        _savedWhitesTime = _whitesTime;
        _savedBlacksTime = _blacksTime;
        _game = bishop.Game(variant: bishop.Variant.standard());
        await setSquaresState();
        print('searchGame: Initialized board, FEN=${_game.fen}, PlayState=${_state.state}, whiteTime=${_whitesTime.inSeconds}s, blackTime=${_blacksTime.inSeconds}s');
        ApiService.initializeSocket(token);
        ApiService.joinGameRoom(_gameId);
        initSocketListeners(context);
        onSuccess();
  ...
  void initSocketListeners(BuildContext context) {
    ...
    ApiService.onOpponentJoined(context, (data) {
      ...
      try {
        ...
        _opponentName = data['opponentName']?.toString() ?? 'Opponent';
        _opponentImage = data['opponentImage']?.toString() ?? '';
        _opponentRating = int.tryParse(data['opponentRating']?.toString() ?? '1200') ?? 1200;
        _whitesTime = Duration(seconds: int.tryParse(data['whiteTime']?.toString() ?? '600') ?? 600);
        _blacksTime = Duration(seconds: int.tryParse(data['blackTime']?.toString() ?? '600') ?? 600);
        _savedWhitesTime = _whitesTime;
        _savedBlacksTime = _blacksTime;
        _isPlaying = true;
        _userId = context.read<AuthProvider>().userId ?? '';
        isHumanWhite = data['creatorId']?.toString() == _userId;
        _player = _isHumanWhite ? Squares.white : Squares.black;
        _game = bishop.Game(variant: bishop.Variant.standard());
        _state = SquaresState(
          board: _game.squaresState(_player).board,
        ...
        print('onOpponentJoined: isHumanWhite=$_isHumanWhite, PlayState=${_state.state}, LegalMoves=${_state.moves.length}, FEN=${_game.fen}, whiteTime=${_whitesTime.inSeconds}s, blackTime=${_blacksTime.inSeconds}s');
        if (context.mounted) {
          Navigator.pushNamedAndRemoveUntil(
            context,
            Constants.gameScreen,
        ...
        print('onOpponentJoined: Error=$e');
        showSnackBar(context: context, content: 'Failed to process opponent join: $e');
      }
    });
    ...
    ApiService.onMoveReceived(context, (data) {
      ...
      try {
        ...
        final isWhiteMove = data['isWhite'] == true;
        _game.loadFen(fen);
        final legalMoves = _game.generateLegalMoves().map((m) => _bishopMoveToSquaresMove(m)).toList();
        _state = SquaresState(
          board: _game.squaresState(_player).board,
        ...
        print('onMoveReceived: Error=$e');
        showSnackBar(context: context, content: 'Failed to process move: $e');
      }
    });
    ...
    ApiService.onGameOver(context, (data) {
      ...
      try {
        final reason = data['reason']?.toString() ?? 'draw';
        final winnerSide = data['winnerSide']?.toString();
        bool userWon = false;
        if (winnerSide != null) {
        ...
        print('onGameOver: Error=$e');
        showSnackBar(context: context, content: 'Failed to process game over: $e');
      }
    });
    ...
    ApiService.socket?.on('rematch', (data) {
      ...
      try {
        _gameId = data['gameId']?.toString() ?? '';
        _joinCode = data['joinCode']?.toString() ?? '';
        _whitesTime = Duration(seconds: int.tryParse(data['whiteTime']?.toString() ?? '600') ?? 600);
        _blacksTime = Duration(seconds: int.tryParse(data['blackTime']?.toString() ?? '600') ?? 600);
        _savedWhitesTime = _whitesTime;
        _savedBlacksTime = _blacksTime;
        ...
        showSnackBar(context: context, content: 'Failed to process rematch: $e');
      }
    });
    ...
    ApiService.onGameRemoved((data) {
      ...
      _availableGames.removeWhere((game) => game['gameId']?.toString() == data['gameId']?.toString());
      notifyListeners();
    });
  }
