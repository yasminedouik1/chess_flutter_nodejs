...
import 'package:chess/chess.dart' as ch; // Import chess.dart for FEN parsing
...
set gameId(String? value) {
  ...
  // Convert FEN to List<String> for squares package
  List<String> _fenToBoard(String fen) {
    final chess = ch.Chess.fromFEN(fen);
    final board = List<String>.filled(64, '');
    for (int i = 0; i < 64; i++) {
      final square = chess.get(squareToAlgebraic(i));
      board[i] = square != null ? _pieceToString(square['type'], square['color']) : '';
    }
    return board;
  }

  // Convert piece type and color to squares-compatible string
  String _pieceToString(String type, String color) {
    final pieceMap = {
      'p': color == 'w' ? 'P' : 'p',
      'n': color == 'w' ? 'N' : 'n',
      'b': color == 'w' ? 'B' : 'b',
  ...
  // Convert UCI move to squares Move
  Move _uciToMove(String uci) {
    final from = algebraicToIndex(uci.substring(0, 2));
    final to = algebraicToIndex(uci.substring(2, 4));
    final promotion = uci.length > 4 ? uci[4].toLowerCase() : null;
    return Move(
      from: from,
      to: to,
      promotion: promotion != null ? _pieceToString(promotion, 'w') : null,
    );
  }
  ...
  //       timer.cancel();
  //       ApiService.socket?.emit('timeout', {'gameId': _gameId, 'timedOutUserId': _isHumanWhite ? _userId : _opponentId});
  ...
  //       timer.cancel();
  //       ApiService.socket?.emit('timeout', {'gameId': _gameId, 'timedOutUserId': _isHumanWhite ? _opponentId : _userId});
  //       gameOverDialog(context: context, timeOut: true, userWon: _isHumanWhite, onNewGame: onNewGame, reason: 'timeout');
  //     }
  //     notifyListeners();
  //   });
  ...
  //         context: context,
  //         timeOut: false,
  //         userWon: (_game.winner == 0) == _isHumanWhite,
  //         onNewGame: onNewGame,
  //         reason: null,
  //       );
  //     }
  ...
  //   try {
  //     if (isJoin && joinGameId != null) {
  //       final data = await ApiService.joinGame(token: token, gameId: joinGameId);
  //       _gameId = joinGameId;
  //       ApiService.initSocket(_gameId);
  //       _opponentId = data['game']['creatorId'];
  //       _opponentName = data['game']['creatorName'];
  //       _opponentImage = data['game']['creatorImage'];
  //       _opponentRating = data['game']['creatorRating'];
  //       _isHumanWhite = false;
  //       _isPlaying = true;
  ...
  //       Navigator.pushNamed(context, Constants.gameScreen);
  //     } else {
  //       _gameId = await ApiService.createGame(
  //         token: token,
  //         whiteTime: int.parse(_savedWhitesTime.inMinutes.toString()),
  //         blackTime: int.parse(_savedBlacksTime.inMinutes.toString()),
  //         increment: _incrementalValue,
  //       );
  //       ApiService.initSocket(_gameId);
  //       _isHumanWhite = true;
  //       initSocketListeners(context);
  //       startWaitingTimer(context);
  //       onSuccess();
  //     }
  //   } catch (e) {
  //     onFail(e.toString());
  ...
  // void startWaitingTimer(BuildContext context) {
  //   _waitingTimer = Timer(const Duration(seconds: 90), () async {
  //     if (!_isPlaying) {
  //       await cancelGame(context);
  //       showSnackBar(context: context, content: 'No opponent found. Game cancelled.');
  //       Navigator.pop(context);
  //     }
  //   });
  // }
  ...
  //     await ApiService.deleteGame(token: token, gameId: _gameId);
  //     _waitingTimer?.cancel();
  //     _isPlaying = false;
  //     resetPvPFields();
  //     notifyListeners();
...
void initSocketListeners(BuildContext context) {
  ...
  ApiService.onMoveReceived(context, (data) {
    ...
    final chess = ch.Chess.fromFEN(fen); // Use chess.dart to parse FEN
    if (state.board.fen != fen) {
      _state = BoardState(
        board: chess, // Update with parsed chess board
        moves: chess.generate_moves().map((m) => Move.fromUci(m.toString())).toList(),
        state: isWhiteMove && player == Squares.black || !isWhiteMove && player == Squares.white
            ? PlayState.ourTurn
            : PlayState.theirTurn,
      ) as SquaresState;
      notifyListeners();
    ...
    if (checkmate(fen, context)) {
      gameOverDialog(
        context: context,
        timeOut: false,
        userWon: (isWhiteMove && player == Squares.black) || (!isWhiteMove && player == Squares.white),
        onNewGame: () {},
        reason: 'checkmate',
      );
    }
