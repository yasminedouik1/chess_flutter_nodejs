...
io.on('connection', (socket) => {
  ...
  socket.on('move', async ({ gameId, move, isWhite, fen }) => {
    ...
    try {
      ...
      const from = squareToAlg(fromSquare);
      const to = squareToAlg(toSquare);
      let promotion;
      if (move.includes('[')) {
        const extras = move.split('[')[1].split(']')[0].split(',');
        promotion = extras[0] || 'q';
      }
      const chess = new Chess(game.fen);
      const validMove = chess.move({ from, to, promotion });
      if (!validMove) {
      ...
      if (isWhite) {
        game.whiteTime = Math.max(0, game.whiteTime + game.increment);
      } else {
        game.blackTime = Math.max(0, game.blackTime + game.increment);
      }
      await game.save();
      ...
      if (chess.isGameOver()) {
        ...
        if (chess.isCheckmate()) {
          winnerId = isWhite ? game.creatorId : game.opponentId;
  ...
  socket.on('resign', async ({ gameId, resignedUserId }) => {
    const Game = require('./models/Game');
    ...
    try {
      ...
      game.isPlaying = false;
      await game.save();
      const winnerId = game.creatorId === resignedUserId ? game.opponentId : game.creatorId;
      io.to(gameId).emit('game_over', { winnerId, reason: 'resign' });
      if (winnerId) {
        const winner = await User.findById(winnerId);
        const loser = await User.findById(resignedUserId);
        if (winner && loser) {
          const expectedWinner = 1 / (1 + Math.pow(10, (loser.playerRating - winner.playerRating) / 400));
          const expectedLoser = 1 - expectedWinner;
          const K = 32;
          winner.playerRating = Math.round(winner.playerRating + K * (1 - expectedWinner));
          loser.playerRating = Math.round(loser.playerRating + K * (0 - expectedLoser));
          await winner.save();
          await loser.save();
        }
      }
